const db = {
    ...
    querys: {
        ...
        actividades : {
            ...
            getCandidatosActividad : 'SELECT U.* FROM Usuario U, Participacion P,(SELECT U2.*, COUNT(*) as numActividades FROM Usuario U2, InformeSemanal Inf2, Actividad A2 WHERE U2.nickUsuario = Inf2.nickUsuario AND A2.nombreActividad = Inf2.nombreActividad AND A2.nombreProyecto = Inf2.nombreProyecto AND A2.nombreActividad = ? AND A2.nombreProyecto = ? AND A2.fechaInicio = (SELECT A3.fechaInicio FROM Actividad A3 WHERE A3.nombreActividad = A2.nombreActividad) GROUP BY U2.nickUsuario) temp, Proyecto Pr WHERE U.nickUsuario = P.nickUsuario AND Pr.nombreProyecto = P.nombreProyecto AND U.nickUsuario NOT IN (SELECT Inf.nickUsuario FROM InformeSemanal Inf,Actividad A WHERE A.nombreActividad = Inf.nombreActividad AND A.nombreProyecto = Inf.nombreProyecto AND A.nombreActividad = ? AND A.nombreProyecto = ?)AND P.rol = (SELECT A2.rol FROM Actividad A2 WHERE A2.nombreActividad = ? AND A2.nombreProyecto = ?) AND temp.numActividades < 4 AND Pr.nombreProyecto = ? UNION SELECT U.* FROM Usuario U, Participacion P, Proyecto Pr, Actividad A WHERE U.nickUsuario = P.nickUsuario AND Pr.nombreProyecto = P.nombreProyecto AND Pr.nombreProyecto = A.nombreProyecto AND A.nombreActividad = ? AND Pr.nombreProyecto = ? AND P.rol = A.rol AND U.nickUsuario NOT IN (SELECT Inf.nickUsuario FROM InformeSemanal Inf)',
